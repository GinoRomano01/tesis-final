...
class StockModel extends Model {
    ...
    // ══ EXCEL: RESOLVER TIPOS POR NOMBRE (usado al editar por Id) ═════════════

    public function buscarIdTipoMadera(string $nombre): ?int {
        $row = Db::query("
            SELECT Id FROM tipodemadera WHERE LOWER(TRIM(Nombre)) = LOWER(TRIM(?))
        ", [$nombre])->fetch();
        return $row ? (int)$row['Id'] : null;
    }

    public function buscarIdTipoMaterial(string $nombre): ?int {
        $row = Db::query("
            SELECT Id FROM tipodematerial WHERE LOWER(TRIM(Nombre)) = LOWER(TRIM(?))
        ", [$nombre])->fetch();
        return $row ? (int)$row['Id'] : null;
    }

    public function buscarIdTipoCorte(string $nombre): ?int {
        $row = Db::query("
            SELECT Id FROM tipodecorte WHERE LOWER(TRIM(Nombre)) = LOWER(TRIM(?))
        ", [$nombre])->fetch();
        return $row ? (int)$row['Id'] : null;
    }
    ...
    public function editarInsumo(int $id, array $d): bool {
        ...
        ", [
            trim($d['Descripcion']),
            $d['IdTipodeMaterial'] !== null ? (int)$d['IdTipodeMaterial'] : null,
            $d['IdTipodeCorte']    !== null ? (int)$d['IdTipodeCorte']    : null,
            (float) ($d['PrecioUnitario'] ?? 0),
            $id,
        ]);
        return true;
